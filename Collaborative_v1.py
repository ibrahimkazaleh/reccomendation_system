...
class NCF(nn.Module):
    def __init__(self, num_users: int, num_items: int, dim: int = 64):
        ...
        # init
        nn.init.normal_(self.user_emb.weight, std=0.001)
...
class RecommenderSystem:
    ...
    def recommend(self, user_idx: int, top_n: int = 5, seen_items: Optional[set] = None) -> List[int]:
        ...
        scores = torch.sigmoid(logits).squeeze().cpu().numpy()  # Use squeeze() to convert to 1D
        ...
        return top_indices.tolist(),scores,logits
...
if __name__ == "__main__":
    ...
    recs_idx = rec.recommend(0, top_n=2, seen_items=seen_sets.get(u, set()))
    recs_idx = rec.recommend(0, top_n=2)

    for n in range(0, 4):
        print(n)
        recs_idx = rec.recommend(n, top_n=3, seen_items=seen_sets.get(u, set()))
...
# المستخدمين والعناصر
users = [chr(97+i) for i in range(20)]  # ['a','b',...,'j']
items = list(range(100, 120))  # [100,...,109]

# إنشاء بيانات التقييمات (Ratings)
ratings_data = []
for _ in range(100):  # 50 تقييم عشوائي
    ratings_data.append({
...
# u = list(user2idx.values())[0]  # أول مستخدم كمثال
# recs_idx ,scores = rec.recommend(0, top_n=2, seen_items=seen_sets.get(u, set()))
# print(recs_idx ,scores)

# recs_idx = rec.recommend(5, top_n=2)

for n in range(0, 8):
    print(n)
    recs_idx ,scores,l = rec.recommend(n, top_n=3, seen_items=seen_sets.get(n, set()))
    # print(scores)
    print(f"Top-{n} recommendations:",recs_idx)
